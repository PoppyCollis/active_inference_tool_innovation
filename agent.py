...
class Agent(object):
    ...
    def __init__(
        ...
        sampling_mode = "full", # whether to sample from full posterior over policies ("full") or from marginal posterior over actions ("marginal")
        ...
        self.policies_restriction=policies_restriction
        self.gamma = gamma
        self.alpha = alpha
    ...
    def _construct_policies(self):
        ...
        policies =  control.construct_policies(
            self.num_states, self.num_controls, self.policy_len, self.control_fac_idx, restriction=self.policies_restriction
        )
    ...
    def infer_policies(self):
        ...
        if self.inference_algo == "VANILLA":
            q_pi, G, G1, G2,G3 = control.update_posterior_policies(
                self.qs,
                self.A,
                self.B,
                self.C,
            ...
            q_pi, G, G1, G2,G3 = control.update_posterior_policies_full(
                future_qs_seq,
                self.A,
                self.B,
                self.C,
    ...
    def infer_policies_factorized_expand_G(self):
        """
        Perform policy inference by optimizing a posterior (categorical) distribution over policies.
        This distribution is computed as the softmax of ``G * gamma + lnE`` where ``G`` is the negative expected
        free energy of policies, ``gamma`` is a policy precision and ``lnE`` is the (log) prior probability of policies.
        ...
        if self.inference_algo == "VANILLA":
            q_pi, G,G1,G2,G3 = control.update_posterior_policies_factorized_expand_G(
                self.qs,
                self.A,
                self.B,
                self.C,
    ...
    def sample_action(self):
        ...
        if self.sampling_mode == "marginal":
            ...
            action, policy_idx = control.sample_policy(
                self.q_pi, self.policies, self.num_controls, action_selection = self.action_selection, alpha = self.alpha
            )
